    ...
    public <T> void update(T obj, int row) {
        ...
        if(obj instanceof Drug)
        {
            ...
            if (rowIndex >= 0) {
                if(this.employeecurrent.getRoleName().equals(RoleName.Admin.toString()) || this.employeecurrent.getRoleName().equals(RoleName.Pharmacist.toString()))
                {
                    if (showMessage("Bạn đang đi đến giao diện chỉnh sửa thông tin thuốc : " + object.getDrugName()+ "(ID: " + object.getID()+ ")", TypeNotification.Default)) {
                        // Chuyển giao diện sang Edit
                        //Drug_Dao object_Dao = new Drug_DaoImpl();
        ...
        if(obj instanceof Service)
        {
            ...
            if (rowIndex >= 0) {
                if(this.employeecurrent.getRoleName().equals(RoleName.Admin.toString()) || this.employeecurrent.getRoleName().equals(RoleName.Pharmacist.toString()))
                {
                    if (showMessage("Bạn đang đi đến giao diện chỉnh sửa thông tin thuốc : " + object.getServiceName()+ "(ID: " + object.getID()+ ")", TypeNotification.Default)) {
                        // Chuyển giao diện sang Edit
                        //Drug_Dao object_Dao = new Drug_DaoImpl();
